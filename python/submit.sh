...
#
# - 사용법
# 1. 사전 입력값을 사용자의 정보로 변경합니다.
# 2. 인수와 함께 스크립트를 실행합니다.
#    ```
...
# 검증용 정규표현식
###############################################################################
TEAM_NUMBER_RANGE_REGEXP="^([1-9]|1[0-6])$"   # 1 ~ 16: 팀 번호 범위
DAY_RANGE_REGEXP="([1-9]|1[0-5])"             # 1 ~ 15: 일차 범위
PROBLEM_NUMBER_REGEXP="([1-9]|1[0-2])"        # 1 ~ 12: 문제 번호 범위
FILE_EXT_REGEXP="^(py|java)$"                 # py 또는 java: 언어별 확장자
OS_WINDOWS_REGEXP="^(MINGW32|MINGW64|CYGWIN)" # Windows
...
if [[ ! "${TEAM_NUMBER}" =~ ${TEAM_NUMBER_RANGE_REGEXP} ]]; then
	echo "Error: 팀 번호가 옳지 않습니다."
	echo "       팀 범위: 1 ~ 16"
	exit 1
fi

if [[ ! "${LANG_EXTENSION}" =~ ${FILE_EXT_REGEXP} ]]; then
...
if [[ ! "${day_problem_number}" =~ ^${DAY_RANGE_REGEXP}-${PROBLEM_NUMBER_REGEXP}$ ]]; then
...
charset="UTF-8"
if [[ $(uname -s) =~ ${OS_WINDOWS_REGEXP} ]]; then
	charset="EUC-KR"
fi
